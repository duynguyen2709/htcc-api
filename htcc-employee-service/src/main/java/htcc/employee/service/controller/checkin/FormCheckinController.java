...
@RestController
@Log4j2
public class FormCheckinController {

    @Autowired
    private CheckInBuzService checkInBuzService;

    @ApiOperation(value = "Điểm danh bằng form", response = BaseResponse.class)
    @PostMapping("/checkin/form")
    public BaseResponse checkInByForm(@ApiParam(value = "[Body] Thông tin điểm danh vào", required = true)
        ...
        model.setSubType(CheckinSubTypeEnum.FORM.getValue());
        model.setStatus(2);
        ...
        try {
            ...
            log.error(String.format("[checkInByForm] [%s] ex", StringUtil.toJsonString(model)), e);
